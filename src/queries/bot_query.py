...
class BotQuery:
    ...
    async def setup_bot_db(self, db_url: str) -> None:
        ...
        bot_user_state_table = self._define_bot_user_state_table(metadata)
        ...
        self._db = BotDBContext(
            ...
            bot_user_state_table=bot_user_state_table,
        )
    ...
    async def insert_or_update_user_state(
        ...
        """
        Insert user state data into the table or update if it conflicts,
        update all columns on conflict except the table pk and the 'created_at' column.
        """
        db = self._get_db()
        current_dt = self._get_current_datetime()
        pk_names = {pk.name for pk in db.bot_user_state_table.primary_key.c}
        excluded_columns = {"created_at", *pk_names}

        async with db.engine.begin() as conn:
            stmt = insert(db.bot_user_state_table).values(
                chat_id=user_state_data.chat_id,
            ...
            upsert_stmt = stmt.on_conflict_do_update(
                ...
                set_={
                    col.name: stmt.excluded[col.name]
                    for col in db.bot_user_state_table.c
                    if col.name not in excluded_columns
                },
            ).returning(db.bot_user_state_table.c.created_at)
            result = await conn.execute(upsert_stmt)
            row = result.fetchone()
            if row and row.created_at == current_dt:
                logger.debug(f"user state {user_state_data.chat_id} inserted")
                return
            logger.debug(f"user state {user_state_data.chat_id} updated")
    ...
    def _define_bot_user_state_table(self, metadata: MetaData) -> Table:
        return Table(
            "bot_user_state",
            metadata,
            Column("chat_id", Integer(), primary_key=True),
